...
#include <ros.h>
#include <jenga_msgs/SensorData.h>
...
jenga_msgs::SensorData sensor_msg;
jenga_msgs::EndEffectorFeedback feedback_msg;
ros::Publisher range_publisher("/tool/range", &sensor_msg);
ros::Publisher probe_publisher("/tool/probe", &sensor_msg);
...
void publishProbeMessage()
{
  ...
  sensor_msg.header.stamp = nh.now();
  sensor_msg.data = force;
  probe_publisher.publish(&sensor_msg);
}
